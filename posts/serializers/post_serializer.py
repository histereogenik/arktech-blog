...
from posts.models import Post, ALLOWED_TAGS
...
class PostSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Post
        fields = [
            'id', 'title', 'subtitle', 'author', 'author_name',
            'tags', 'image', 'paragraphs', 'cta',
    ...
    def validate_paragraphs(self, value):
        ...
        for item in value:
            if not isinstance(item, dict):
                raise serializers.ValidationError("Each paragraph must be a dictionary.")
            if 'content' not in item:
                raise serializers.ValidationError("Each paragraph must have a 'content' field.")
        return value
